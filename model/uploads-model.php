...
function getThumbnailImage($invId){
    $db = phpmotorsConnect();
    $sql = 'SELECT imgPath, imgName FROM images JOIN inventory ON inventory.invId = images.invId WHERE ( imgPrimary = 0 AND imgPath LIKE "%-tn%") AND inventory.invId = :invId';
    $stmt = $db->prepare($sql);
